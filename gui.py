...
def heuristic(game:Game):
    ...
    for i, box in enumerate(boxes_loc):
        ...
        for portal in portals_loc:
            player_portal_path = min(manhattan_dist(player_loc, portal[0]) + manhattan_dist(portal[1], box), manhattan_dist(player_loc, portal[1]) + manhattan_dist(portal[0], box))
            box_portal_path = min(manhattan_dist(box, portal[0]) + manhattan_dist(portal[1], goals_loc[i]), manhattan_dist(box, portal[1]) + manhattan_dist(portal[0], goals_loc[i]))
            if player_portal_path < player_to_box:
                player_to_box = player_portal_path
            if box_portal_path < box_to_goal:
                box_to_goal = box_portal_path
                
        if len(portals_loc) > 1:
            player_portal_path = min(
            manhattan_dist(player_loc, portals_loc[0][0]) + manhattan_dist(portals_loc[0][1], portals_loc[1][0]) + manhattan_dist(portals_loc[1][1], box), 
            manhattan_dist(player_loc, portals_loc[0][0]) + manhattan_dist(portals_loc[0][1], portals_loc[1][1]) + manhattan_dist(portals_loc[1][0], box), 
            manhattan_dist(player_loc, portals_loc[0][1]) + manhattan_dist(portals_loc[0][0], portals_loc[1][0])+ manhattan_dist(portals_loc[1][1], box), 
            manhattan_dist(player_loc, portals_loc[0][1]) + manhattan_dist(portals_loc[0][0], portals_loc[1][1]) + manhattan_dist(portals_loc[1][0], box))
            
            if (player_portal_path < player_to_box):
                player_to_box = player_portal_path
            box_portal_path = min(
            manhattan_dist(box, portals_loc[0][0]) + manhattan_dist(portals_loc[0][1], portals_loc[1][0]) + manhattan_dist(portals_loc[1][1], goals_loc[i]), 
            manhattan_dist(box, portals_loc[0][0]) + manhattan_dist(portals_loc[0][1], portals_loc[1][1]) + manhattan_dist(portals_loc[1][0], goals_loc[i]), 
            manhattan_dist(box, portals_loc[0][1]) + manhattan_dist(portals_loc[0][0], portals_loc[1][0])+ manhattan_dist(portals_loc[1][1], goals_loc[i]), 
            manhattan_dist(box, portals_loc[0][1]) + manhattan_dist(portals_loc[0][0], portals_loc[1][1]) + manhattan_dist(portals_loc[1][0], goals_loc[i]))
            if (box_portal_path < box_to_goal):
                box_to_goal = box_portal_path
                
        box_to_goals[i] = box_to_goal
        player_to_boxes[i] = player_to_box
    
    return sum(box_to_goals) + min(player_to_boxes)

# TODO: Must return moves, number of visited states
def solver_astar(game_map, heuristic_func=heuristic, weight=4.5):
    game = Game(game_map)
    initial_state = State(game.get_player_position(), game.get_box_locations(), "")
    minheap = []
